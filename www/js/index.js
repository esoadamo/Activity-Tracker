...
function generateTable(year, month, day, daysToShow=null) {
  ...
  */
  ...
  let dayGeneratedData = document.querySelector('#dayGeneratedData');

  // On date change, show selected day
  let shownDate = document.querySelector('#shownDate');
...
app.initialize();
generateTable(2018, 1, 4, 31);
let shownDate = document.querySelector('#shownDate');
shownDate.addEventListener('change', ()=>{
  let dateData = shownDate.value.split('-'); // yyyy-mm-dd format
  generateTable(parseInt(dateData[0]), parseInt(dateData[1]), parseInt(dateData[2]));
});
